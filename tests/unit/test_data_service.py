...
# 导入fixtures
try:
    from test_data import csv_test_data, empty_csv_file, malformed_csv_file, sample_user_data, test_output_dir
except ImportError:
...
class TestDataProcessor:
    ...
    def test_process_single_row_dataframe(self, processor):
        """测试处理单行DataFrame"""
        single_row_df = pd.DataFrame({"name": ["Alice"], "age": [25], "city": ["New York"]})
