...
from src.business.models import DataRecord, get_data_validator
...
class TestBusinessModels:
    ...
    def test_data_record_creation(self):
        """测试数据记录创建"""
        record = DataRecord(name="测试记录", value=100.0, category="测试分类")
    ...
    def test_data_record_validation(self):
        ...
        # 有效数据
        valid_data = {"name": "测试", "value": 50.0, "category": "测试"}
        result = validator.validate_data_record(valid_data)
        assert result.is_valid

        # 无效数据
        invalid_data = {"name": "", "value": -10.0, "category": "测试"}
