...
from src.infrastructure.database import BaseModel, db_manager, initialize_databases, transactional, with_db_session
...
def demo_create():
    ...
    with db_manager.get_db_session() as db:
        ...
        # 创建分类
        category1 = Category.create(db, name="技术文章", description="技术相关的文章分类", sort_order=1)
...
def demo_update():
    ...
    with db_manager.get_db_session() as db:
        ...
        # 更新文章
        updated_article = Article.update(db, article_id, views=article.views + 1, status="published")
...
def create_article_with_decorator(db: Session, title: str, content: str, author: str):
    ...
    Args:
        ...
        author: 作者
    """
    article = Article.create(db, title=title, content=content, author=author, status="draft")
...
def demo_multi_database():
    ...
    with db_manager.get_db_session("default") as db:
        article1 = Article.create(
            db, title="默认数据库文章", content="这篇文章存储在默认数据库中", author="作者A", status="published"
        )
        db.commit()
    ...
    with db_manager.get_db_session("analytics") as db:
        article2 = Article.create(
            db, title="分析数据库文章", content="这篇文章存储在分析数据库中", author="作者B", status="published"
        )
        db.commit()
    ...
    with db_manager.get_db_session("logs") as db:
        article3 = Article.create(
            db, title="日志数据库文章", content="这篇文章存储在日志数据库中", author="作者C", status="published"
        )
        db.commit()
