...
from ..models import get_data_validator
...
class DataProcessor:
    ...
    def _validate_dataframe(self, df: pd.DataFrame) -> None:
        ...
        for idx, row in df.iterrows():
            data_dict = {"name": str(row["name"]), "age": int(row["age"]), "city": str(row["city"])}
    ...
    def process_data_stream(
        ...
        try:
            for data in data_stream:
                ...
                # 处理数据
                processed_data = {**data, "processed": True, "processed_at": datetime.now().isoformat()}
