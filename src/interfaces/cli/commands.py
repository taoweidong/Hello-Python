...
from ...business.services import get_analysis_service, get_data_processing_service
...
@click.pass_context
def cli(ctx, env_file: Optional[str] = None):
    """数据分析项目命令行接口"""
    try:
        # 初始化应用
...
@click.option('--processing-type', default='normalization', 
...
@click.option('--input-file', required=True, help='输入CSV文件路径')
@click.option('--analysis-type', default='statistical',
              type=click.Choice(['statistical', 'trend']),
              help='分析类型')
@click.pass_context
